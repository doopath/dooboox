export class Searcher {
  ...
  public constructor(string: string, elements: Array<string>) {
    this.searchString = this.removeSpecialSymbols(string.toLowerCase())
    this.result = ""
  ...
  // ( fast search replaces any typecases symbols by same but lowercase, but it's faster )
  ...
  private parse = (
    ...
    if (result === "") {
      ...
      if (foundToken !== null) {
        let offString = string.substring(Number(foundToken.index) + token.length)
        result = result.substring(0, foundToken.index) + this.mark(foundToken[0]) + offString
        this.parse(token, offString, result)
      } else
        return result
    } else {
      let foundToken: RegExpMatchArray | null = string.match(new RegExp(token, "i"))
  ...
  private removeSpecialSymbols = (string: string): string => {
    // Here I'm gonna remove any stuff like shitsymbols else it will throw an error
